...
export class ResidentSelfOnboardingService {
    ...
    private async validateApartmentTypeForEstate(
        ...
        if (applyApartmentType && !apartmentTypeId) {
            return error(
        ...
        const apartmentType = await this.prisma.apartmentType.findFirst({
            where: { id: apartmentTypeId, estateId, active: true },
        });
    ...
    async register(dto: ResidentSelfOnboardingDto) {
        ...
        const existing = await this.prisma.resident.findFirst({
            where: {
                OR: [{ email: dto.email }, { phone: whatsappPhone }],
            },
        });
        ...
        const appDownloadLink =
            process.env.APP_DOWNLOAD_LINK ?? 'https://oderasafe.ddsafe.tech';

        await this.validateApartmentTypeForEstate(
            estate.id,
            dto.apartmentTypeId,
        );
