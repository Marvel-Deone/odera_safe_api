...
export class ResidentAssociateController {
    constructor(
        private readonly residentAssociateService: ResidentAssociateService,
    ) {}
    ...
    @Get('dashboard')
    @ApiOperation({
        summary: 'Get co-resident dashboard with gate QR and passcode',
    })
    getCoResidentDashboard(@CurrentUser() user: any) {
        return this.residentAssociateService.getCoResidentDashboard(user.id);
    }
