...
export class FinanceController {
  ...
  initializeWalletFunding(@CurrentUser() user: any, @Body() dto: FundWalletDto) {
  ...
  requestWithdrawal(@CurrentUser() user: any, @Body() dto: RequestWithdrawalDto) {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.ADMIN, Role.SUPER_ADMIN)
  @ApiOperation({ summary: 'Approve withdrawal, optionally initiating Paystack transfer' })
  approveWithdrawal(
    @CurrentUser() user: any,
    @Param('withdrawalId') withdrawalId: string,
    @Query('bankCode') bankCode?: string,
  ) {
    return this.financeService.approveWithdrawal(user.id, withdrawalId, bankCode)
  }
