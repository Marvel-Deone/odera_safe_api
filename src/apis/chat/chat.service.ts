...
export class ChatService {
    ...
    private async ensureDefaultRooms(estateId: string) {
        await Promise.all([
            ...
            this.ensureEstateRoom(
                estateId,
                ChatRoomType.SECURITY,
                'Estate Security',
            ),
            this.ensureEstateRoom(
                estateId,
                ChatRoomType.ADMIN_ONLY,
                'Estate Admins',
            ),
        ]);
    }

    private async canAccessRoom(user: ChatUser, room: ChatRoom) {
        ...
        if (room.type === ChatRoomType.STREET) {
            return (
                ...
                room.streetId === user.resident?.streetId
            );
        }
        ...
        if (room.type === ChatRoomType.INCIDENT) {
            const participant =
                await this.prisma.incidentChatParticipant.findFirst({
                    where: {
                        roomId: room.id,
                        userId: user.id,
                    },
    ...
    async getRooms(userId: string) {
        ...
        if (this.isAdmin(user)) {
            ...
            await Promise.all(
                streets.map((street) =>
                    this.ensureStreetRoom(user.estateId, street.id),
                ),
            );

            const rooms = await this.prisma.chatRoom.findMany({
                where: { estateId: user.estateId },
                include: { street: true, incident: true },
    ...
    async sendMessage(userId: string, roomId: string, dto: SendMessageDto) {
        ...
        if (room.closedAt) {
            return error('Chat Closed', 'This incident chat is read-only', 403);
        }
